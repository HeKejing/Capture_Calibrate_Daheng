...
bool KJCalibration::calibrate()
{
    ...
    cv::FileStorage fs(result_yml, cv::FileStorage::WRITE);
    if (!fs.isOpened()) {
        cout << "fs.isOpened() failed." <<endl;
    }
    else {
        fs << "camL_K" << cameraMatrix_L << "camL_kc" << Cd_L
            << "camR_K" << cameraMatrix_R << "camR_kc" << Cd_R
            << "R" << R << "T" << T << "E" << E << "F" << F << "camL_error" << camL_error << "camR_error" << camR_error << "stereo_error" << cam_two_error;
        fs.release();
    }
    cout << "CamL_error : " << camL_error << endl;
    cout << "CamR_error : " << camR_error << endl;
